...
namespace Rendering
{
	...
	{
	public:
		PointLightDemo(Library::Game& game);
		PointLightDemo(Game & game, const std::shared_ptr<Library::Camera>& camera);
	...
	private:
		...
		struct CBufferPerFrameVS
		{
			...
			CBufferPerFrameVS() : CameraDirection(), Padding() {}
			CBufferPerFrameVS(const DirectX::XMFLOAT3 cd) : CameraDirection(cd), Padding() {}
		};
		...
		struct CBufferPerFramePS
		{
			...
			CBufferPerFramePS() : AmbientColor(), LightColor(), Padding() {}
			CBufferPerFramePS( const DirectX::XMFLOAT4 ac, const DirectX::XMFLOAT3 lc) : AmbientColor(ac), LightColor(lc), Padding() {}
		};
