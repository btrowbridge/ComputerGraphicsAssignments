...
namespace Rendering
{
	...
	public:
		...
		SpecularLightingDemo(Game & game, const std::shared_ptr<Library::Camera>& camera);
	...
	private:
		...
		struct CBufferPerFrameVS
		{
			...
			CBufferPerFrameVS() : CameraPosition(), Padding(){}
			CBufferPerFrameVS(const DirectX::XMFLOAT3 cd) : CameraPosition(cd), Padding(){}
		};
