...
void Mesh::Save(OutputStreamHelper& streamHelper)
{
	...
	for (XMFLOAT3& vertex : mData.Vertices)
	{
	...
	for (XMFLOAT3& normal : mData.Normals)
	{
	...
	for (XMFLOAT3& tangents : mData.Tangents)
	{
	...
	for (XMFLOAT3& binormal : mData.BiNormals)
	{
	...
	for (auto uvList : mData.TextureCoordinates)
	{
		streamHelper << static_cast<uint32_t>(uvList->size());
		for (XMFLOAT3& uv : *uvList)
		{
	...
	for (auto vertexColorList : mData.VertexColors)
	{
		streamHelper << vertexColorList->size();
		for (XMFLOAT4& vertexColor : *vertexColorList)
		{
