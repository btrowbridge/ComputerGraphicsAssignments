...
void ModelMaterial::Save(OutputStreamHelper& streamHelper)
{
	streamHelper << mData.Name;

	streamHelper << static_cast<uint32_t>(mData.Textures.size());
	for (auto& texturePair : mData.Textures)
	{
		streamHelper << static_cast<int>(texturePair.first);
		streamHelper << static_cast<uint32_t>(texturePair.second->size());
		for (auto texture : *(texturePair.second))
		{
