...
namespace Library
{
	RTTI_DEFINITIONS(FpsComponent)
		...
		mFrameCount(0), mFrameRate(0), mLastTotalGameTime()
	{
	}
	...
	void FpsComponent::Draw(const GameTime& gameTime)
	{
		...
		fpsLabel << std::setprecision(4) << L"Frame Rate: " << mFrameRate << "    Total Elapsed Time: " << gameTime.TotalGameTimeSeconds().count();
